...
INPUT_FILE=driverset
OUTPUT_PATH=lineCalculate

hadoop jar ${D_HOME}/lib/bigdata-1.0-SNAPSHOT.jar com.haifwu.prince.CalculateFileLength \
    -libjars ${LIBJARS} \
    -Dinput.path=${INPUT_FILE} \
    -Doutput.path=${OUTPUT_PATH} \
    -Dmapred.job.queue.name=pp_risk_dst 
